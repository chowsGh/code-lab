...
import top.clydezhou.lab.demo.spring.config.database.H2DataSourceConfig;
...
/**
 * The type Mybatis h 2 config.
 * 
 * @date 2020.06.16 23:36:26
 */
@Configuration
// 因为使用了两个 sqlSessionFactory 所以使用扫描会报冲突。需要手动配置 mapper
...
public class MybatisH2Config {
    ...
    @Bean
    public UserMapper h2UserMapper() throws Exception {
        MapperFactoryBean<UserMapper> mapperBean = new MapperFactoryBean<>(UserMapper.class);
        mapperBean.setSqlSessionFactory(h2SessionFactory());
     ...
     */
    @Bean
    public SqlSessionFactory h2SessionFactory() throws Exception {
    ...
    public DataSource dataSource() {
        ...
        source.setDriverClassName(h2DataSourceConfig.getDriverClassName());
        source.setJdbcUrl(h2DataSourceConfig.getJdbcUrl());
        source.setUsername(h2DataSourceConfig.getUsername());
        source.setPassword(h2DataSourceConfig.getPassword());
        return source;
    }
}
